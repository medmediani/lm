...
def get_sent_prob(sent, od):
    tokens=[BOS]+[w if w in od[1] else UNK for w in sent.strip().split()]+[EOS]
    l=len(tokens)
    max_ord=max(od.keys())
    return sum(getprob(tokens[:i], od) for i in range (2,min(max_ord, l)))+\
...
if __name__=="__main__":
    ...
    for l in sys.stdin:
        print(get_sent_prob(l.strip(), lm))
